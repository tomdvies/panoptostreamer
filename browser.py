...
# want "Id" tag
want = ["II General Relativity LT23", "II Probability and Measure MT22","II Statistical Physics LT23","II Classical Dynamics MT22"]
folders = [fl for fl in folders if "II" in fl["Name"] and fl["Name"] in want]
course = folders[2]
...
for course in folders:
    ...
    for video in folderreq["d"]["Results"]:
        try:
            ...
            if not os.path.exists(f"rawlectures/{course['Name']}/{name}/"):
                os.makedirs(f"rawlectures/{course['Name']}/{name}/")
            ...
            with ThreadPoolExecutor(max_workers=4) as pool:
                results = pool.map(lambda p: m3u8fetch.download_m3u8(*p),zip(mu3links,
                                                                             [f"rawlectures/{course['Name']}/{name}/out{j}.mp4" for j in range(len(mu3links))]))
